...
@export var ENEMY_SCALE: float = 0.4
...
func _ready():
	...
	forward_ray = RayCast3D.new()
	forward_ray.position = Vector3(0, 0.5, 0)
	forward_ray.target_position = Vector3(0, 0, -1.2)
...
func _physics_process(delta: float):
	...
	if (forward_ray.is_colliding() or is_on_wall()) and turn_cooldown <= 0.0:
		var turns = [PI/2, -PI/2]
		move_direction = move_direction.rotated(Vector3.UP, turns[randi() % turns.size()])
		...
		if abs(move_direction.x) > abs(move_direction.z):
			...
			move_direction.z = sign(move_direction.z)
	...
	# Only move XZ if not suspended in midair (giving it a bit of time to snap to ceiling first)
...
func _check_player_contact():
	...
	# Fallback slide collision check
