...
@export var RAY_COUNT: int = 700         # High density across walls
@export var MAX_SPREAD_DEG: float = 48.0 # Wide angle cone covering most of screen
@export var CENTER_WEIGHT: float = 1.4   # Smooth falloff from center to edges
...
func _ready():
	...
	# Strict alignment with camera lens
	...
	scanner_ray.collide_with_areas = false
...
func _unhandled_input(event):
	...
	# Single-click pulse trigger (Left click or fire_lidar)
	var clicked = false
...
func _fire_circular_scan():
	...
	for i in range(RAY_COUNT):
		# Exponential distribution: concentrates at center but fills wide cone
		...
		# Cast relative to Camera forward vector
